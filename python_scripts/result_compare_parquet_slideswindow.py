...
FLINK_OUTPUT_FILE = "D:\\FlinkWatermarkExp\\experiment_result\\slide_window_result_60.txt"
#FLINK_OUTPUT_FILE="C:\\Users\\Public\\slide_window_result_15.txt"
TIME_COL = 'tpep_pickup_datetime'
TAXI_COL = 'VendorID'
TIP_COL = "tip_amount"
WINDOW_HOURS = 1
SLIDE_MINUTES = 30  # 添加滑动步长
...
# -----------------------------
def get_sliding_windows(ts):
    """
    Given a timestamp ts, return all sliding window start times (hour-window, slide=30min)
    A window is valid if ts is inside [start, start+1h)
    """
    # floor to nearest slide boundary (0 or 30 min)
    base = ts.floor(f"{SLIDE_MINUTES}min")
    
    windows = []
    for k in range(0, WINDOW_HOURS * 60 // SLIDE_MINUTES + 1):
        ws = base - pd.Timedelta(minutes=SLIDE_MINUTES * k)
...
# explode 成多窗口
df["windows"] = df[TIME_COL].apply(get_sliding_windows)
df_expanded = df.explode("windows")

# 聚合 ground truth
ground_truth_df = df_expanded.groupby(["windows", TAXI_COL])[TIP_COL].sum().reset_index()

ground_truth = {
    (row["windows"], row[TAXI_COL]): row[TIP_COL]
